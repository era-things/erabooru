...
func (tq *TagQuery) QueryMedia() *MediaQuery {
...
func (tq *TagQuery) WithMedia(opts ...func(*MediaQuery)) *TagQuery {
	query := (&MediaClient{config: tq.config}).Query()
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withMedia; query != nil {
		if err := tq.loadMedia(ctx, query, nodes,
			func(n *Tag) { n.Edges.Media = []*Media{} },
			func(n *Tag, e *Media) { n.Edges.Media = append(n.Edges.Media, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Media)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.MediaTable)
		s.Join(joinT).On(s.C(media.FieldID), joinT.C(tag.MediaPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.MediaPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.MediaPrimaryKey[1]))

...
func (aq *AttributeQuery) QueryMedia() *MediaQuery {
...
func (aq *AttributeQuery) QueryMediaAttributes() *MediaAttributeQuery {
	query := (&MediaAttributeClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (aq *AttributeQuery) Clone() *AttributeQuery {
	...
	return &AttributeQuery{
		...
		withMediaAttributes: aq.withMediaAttributes.Clone(),
...
func (aq *AttributeQuery) WithMedia(opts ...func(*MediaQuery)) *AttributeQuery {
	query := (&MediaClient{config: aq.config}).Query()
...
func (aq *AttributeQuery) WithMediaAttributes(opts ...func(*MediaAttributeQuery)) *AttributeQuery {
	query := (&MediaAttributeClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withMediaAttributes = query
	return aq
}
...
func (aq *AttributeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Attribute, error) {
	...
	if query := aq.withMedia; query != nil {
		if err := aq.loadMedia(ctx, query, nodes,
			func(n *Attribute) { n.Edges.Media = []*Media{} },
			func(n *Attribute, e *Media) { n.Edges.Media = append(n.Edges.Media, e) }); err != nil {
			return nil, err
		}
	}
	if query := aq.withMediaAttributes; query != nil {
		if err := aq.loadMediaAttributes(ctx, query, nodes,
			func(n *Attribute) { n.Edges.MediaAttributes = []*MediaAttribute{} },
			func(n *Attribute, e *MediaAttribute) { n.Edges.MediaAttributes = append(n.Edges.MediaAttributes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (aq *AttributeQuery) loadMedia(ctx context.Context, query *MediaQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *Media)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Attribute)
	nids := make(map[string]map[*Attribute]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(attribute.MediaTable)
		s.Join(joinT).On(s.C(media.FieldID), joinT.C(attribute.MediaPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(attribute.MediaPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(attribute.MediaPrimaryKey[1]))
	...
	qr := QuerierFunc(func(ctx context.Context, q Query) (Value, error) {
		return query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
			...
			spec.Assign = func(columns []string, values []any) error {
				...
				if nids[inValue] == nil {
					nids[inValue] = map[*Attribute]struct{}{byID[outValue]: {}}
...
func (aq *AttributeQuery) loadMediaAttributes(ctx context.Context, query *MediaAttributeQuery, nodes []*Attribute, init func(*Attribute), assign func(*Attribute, *MediaAttribute)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(mediaattribute.FieldAttributeID)
	}
	query.Where(predicate.MediaAttribute(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(attribute.MediaAttributesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "attribute_id" returned %v for node %v`, fk, n)
		}
		assign(node, n)
	}
	return nil

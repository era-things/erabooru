<script lang="ts">
    ...
    const q = $derived(
        !hasVectorParam && vectorFlag
            ? ''
            : rawQuery
    );
    const vectorSearch = $derived(
        vectorQuery.trim().length > 0 && (vectorFlag || hasVectorParam)
    );
...
<div class="h-screen">
    <TabNav active="media" />
    <MediaGrid query={q} vectorQuery={vectorQuery} page={currentPage} {pageSize} bind:total />
